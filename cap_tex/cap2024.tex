...
\begin{itemize}
  ...
  Note that the KOS strategy is based on a weighted majority vote -- hence why we included it -- but the weights take into account the task's index, thus leading to $n_\text{task}$ weight matrices. Moreover, in practice, the weights can diverge and implementations use a threshold to stop the iterations (see the implementation\footnote{\url{https://github.com/Toloka/crowd-kit/blob/v1.2.1/crowdkit/aggregation/classification/kos.py\#L11}} in \citep{CrowdKit2023}).

\end{itemize}
Note that depending on the strategy, the weights $w_{j,k}$ might not be upper-bounded. Indeed, in WAWA as the weight is the accuracy, it is at most $1$. However, the KOS strategy does not have an upper bound on the weights.
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{./../summary_plot_shap_beeswarm_bluebirds.pdf}
  \caption{Summary of the main contributing workers for the BlueBirds dataset using Shapley values. Each dot is a task with its associated Shapley value. Left: Worker $34$ has the highest contribution to the final label -- either for class $0$ or $1$, followed by worker $31$ and $0$. Then, worker $19$ has better identification for class $0$ than class $1$. Right: Average impact of each worker on the final predicted label by the XGBOOST classifier. This worker's contribution scalar value -- used in \Cref{alg:shap} weight -- does not allow to differentiate between classes contribution.}
...
The accuracy of worker $0$ is $0.80$, worker $34$ is $0.79$, worker $31$ is $0.74$ -- the three main workers contributing to the final aggregation -- and worker $33$ is $0.44$ (random answers).
...
Worker $19$ has a better identification for class $0$ than class $1$.
