...
from crowdkit.aggregation import DawidSkene, GLAD, MACE, MMSR, KOS
...
for data, n_class, n_worker, n_task in tqdm(datasets, desc="Datasets"):
    ...
    for i, (strat, library) in tqdm(enumerate(strategies)):
        ...
        results_crowdkit_pd.to_csv("results_crowdkit_fi.csv")
        print(results_crowdkit_pd.groupby(by=["dataset", "strategy"]).max())
# %%

data = pd.read_csv("results_crowdkit_fi.csv")
# %%
for df, *_ in datasets:
    print()
    print(df)
    print(
        data[data["dataset"] == df]
        .groupby(by=["strategy"])
